...
training_generator = DataGenerator(feature_vector,cleaned_desc,ixtoword,wordtoix,desc_maxlen,vocab,batch_size)
...
for z in range(5000,400000,5):
    image = feature_vector[z].reshape((1,2048))
    img = mpimg.imread("./Dataset 2/train2014/"+cleaned_id[z])
    plt.imshow(img)
    plt.show()
    print("Greedy:",greedySearch(image))
    time.sleep(2)

model2=load_model('./model_weights/model_cap.h5')