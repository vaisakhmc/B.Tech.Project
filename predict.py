...
model=load_model('./model_weights/model_cap.h5')
model_yolo=load_model('./model_weights/model_cap_yolo.h5')
model_yolo_all=load_model('./model_weights/model_total_yolo.h5')
model_yolo.summary()
...
def SearchYoloAll(photo,yolo):
    in_text = 'startseq'
    for i in range(desc_maxlen):
        sequence = [wordtoix[w] for w in in_text.split() if w in wordtoix]
        sequence = pad_sequences([sequence], maxlen=desc_maxlen)
        yhat = model_yolo_all.predict([photo,sequence,yolo], verbose=0)
        yhat = np.argmax(yhat)
        word = ixtoword[yhat]
        in_text += ' ' + word
        if word == 'stopseq':
